...
void cdp_opencpu_demo_task(void *args)
{
    ...
    //阻塞等待云平台的下行数据
    if(cdp_opencpu_demo_sem == NULL)
    	cdp_opencpu_demo_sem = osSemaphoreNew(0xFFFF, 0);
...
void cdp_opencpu_demo_init()
{
    ...
    //创建CDP的demo线程
	g_cdp_opencpu_demo_Handle = osThreadNew ((osThreadFunc_t)(cdp_opencpu_demo_task),NULL,"cdp_opencpu_demo",0x400,osPriorityNormal);
}
#endif
#endif
