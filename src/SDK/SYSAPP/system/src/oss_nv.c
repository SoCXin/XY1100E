...
#if BLE_ENABLE
	xy_flash_erase(NV_FLASH_BLE_BASE,NV_FLASH_BLE_LEN);
#endif
}
...
void nv_restore()
{
	uint8_t backup_mem_lastBytes[12];
#if USE_ADC_CAL
...
ABNORMAL_PROC:
	...
	{
READ_FLASH:
		/* 这里的xy_ftl_read启动时读NV会一次性读取4K，backup mem 最后12字节有其他用途需要保存下来read后再恢复 */
		memcpy(backup_mem_lastBytes, (uint8_t *)BACKUP_MEM_FLAG, sizeof(backup_mem_lastBytes));
		if(xy_ftl_read(NV_FLASH_DSP_VOLATILE_BASE,(unsigned char *)BACKUP_MEM_BASE,XY_FTL_AVAILABLE_SIZE)!= XY_OK)
        {        
        	memcpy((uint8_t *)BACKUP_MEM_FLAG, backup_mem_lastBytes, sizeof(backup_mem_lastBytes));
        	g_dsp_mode_dbg|=1<<11;
...
void check_conflict_nv()
{
	...
	else
	{
		...
		if(g_softap_fac_nv->xtal_32k==1 && g_softap_fac_nv->ps_utcwkup_dis==1 && (g_softap_fac_nv->lpm_standby_enable==1 || g_softap_fac_nv->wfi_enable==1))
			sprintf(at_str + strlen(at_str), "+DBGINFO:NV_ERR:xtal_32k=%d,ps_utcwkup_dis=%d,lpm_standby_enable=%d,wfi_enable=%d\r\n", g_softap_fac_nv->xtal_32k, g_softap_fac_nv->ps_utcwkup_dis, g_softap_fac_nv->lpm_standby_enable, g_softap_fac_nv->wfi_enable);
	}
