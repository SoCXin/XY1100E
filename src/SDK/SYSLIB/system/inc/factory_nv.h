...
typedef struct {
    ...
    char  ver_type;            //##2## software version type;1:telecom operators version,not use WORKLOCK; 2: SDK version;4:jinka SDK;8:CMIOT;16:QUECTEL
    ...
    char  offtime;                   //##0##&& indicate whether do AT+OFFTIME or not when POWER ON by POWER OFF mode;if close,POWER ON will do attach
    char  padding10;                //##0##&& 
	...
	char  hard_dog_mode;            //##0##&&if 0,The system automatically feeds watchdog;1 The system will not automatically feed the dog.
	uint8_t  hard_dog_time;            //##0##&& minutes,set watchdog reload value.if hard_dog_mode = 1,will reset SoC when timeout 
	...
	uint16_t  rc32k_freq;		   //##0## 0,rc32k frequence,only be valid when xtal_32k=1
	char  padding18[2];
