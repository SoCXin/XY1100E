...
#define SAVE_FAC_PARAM(param)	\
		{  \
			if(xy_ftl_write(NV_FLASH_FACTORY_BASE,DSP_FAC_LEN+OFFSET_FAC_PARAM(param),(unsigned char *)&g_softap_fac_nv->param,FAC_PARAM_LEN(param)) != XY_OK) \
			{  \
...
#define SAVE_SOFTAP_FAC()	\
		{  \
			if(xy_ftl_write(NV_FLASH_FACTORY_BASE,DSP_FAC_LEN,(unsigned char *)g_softap_fac_nv,sizeof(softap_fac_nv_t)) != XY_OK) \
			{  \
				xy_flash_erase(NV_FLASH_FACTORY_BASE,NV_FLASH_FACTORY_LEN);  \
				xy_assert(0);  \
			} \
