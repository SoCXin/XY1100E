...
void AutoBaudStart()
{
	//timeout at least 1000/50=20ms,for 2400,5bytes
	...
	while(1)
	{
		switch(g_autobaud_state)
		{
			case AUTOBAUD_STATE_INIT:
			{
				...
				//init systick
				SysTick->LOAD  = (uint32_t)AUTOBAUD_SYSTICK_LOAD_RELOAD;                         /* set reload register */
				...
				if( checkvalue>36 && checkvalue<44)
				{
					...
					{						
						...
						do{
							...
							{
								edge_triggle_cnt++;
								systick_reload_flag = 0;//make sure transmission completed,then timeout start to count
							}
						...
						}while(/*edge_triggle_cnt < 10 && */systick_reload_flag < 2);//enough time for receiving
