...
void set_DSP_work_mode()
{
	...
	//when Power on first,or AT+FASTOFF,cannot erase user RTC
	if(g_softap_var_nv->rtc_snap_sec==0 || g_softap_var_nv->next_PS_sec==0)
	{
		g_dsp_mode_dbg |= 1 << 3;
		//set sharing memory to default,trigger PS to power on
		HWREG(PS_START_SYNC) = PS_DO_ATTACH;
	}
	//ps abnormal, do attach
	else if(g_softap_var_nv->next_PS_sec == 0XFFFFFFFF)
	{
		g_dsp_mode_dbg |= 1 << 16;
		HWREG(PS_START_SYNC) = PS_DO_ATTACH;
	}
	//UTC wakup,TAU/DRX/eDRX timeout or user rtc wakeup(tau/drx/edrx not timeout),do campon
	else if (get_sys_up_stat() == UTC_WAKEUP && g_softap_var_nv->next_PS_sec != 0 && g_softap_var_nv->next_PS_sec >= (unsigned int)(cur_rtc_sec - 1))
	{
	...
	//if PIN wakeup before RTC timeout,do campon
	else if (get_sys_up_stat() == EXTPIN_WAKEUP && g_softap_var_nv->next_PS_sec != 0 && g_softap_var_nv->next_PS_sec >= (unsigned int)(cur_rtc_sec - 1))
	{
		g_dsp_mode_dbg |= 1 << 1;
		HWREG(PS_START_SYNC) = PS_CAMP_ON;
		g_normal_wakeup = 1;
	}
	//if workmode!=0 do dynamic load DSP core,or abnormal val, do attach
	else
	{
		g_dsp_mode_dbg |= 1 << 15;
		HWREG(PS_START_SYNC) = PS_DO_ATTACH;
...
void dynamic_load_up_dsp(int isDump)
{
	...
	//if(s_loadup_dsp_flag == 0)
	{
		...
		{
			if(xy_ftl_read(NV_FLASH_DSP_VOLATILE_BASE,(unsigned char *)BACKUP_MEM_BASE,DSP_VOLATILE_ALL_LEN)!= XY_OK)
			{		 
