...
#ifdef WITH_LWIP
#if IS_DSP_CORE
  memp_free(COAP_RESOURCEATTR, attr);
#else
...
#if IS_DSP_CORE
    coap_free_type( COAP_RESOURCE, obs );
#else
...
#ifdef WITH_LWIP
#if IS_DSP_CORE
  memp_free(COAP_RESOURCE, resource);
#else
...
coap_delete_observer(coap_resource_t *resource, coap_session_t *session,
  ...
  if (resource->subscribers && s) {
    ...
    if (s->query)
      coap_delete_string(s->query);
#if IS_DSP_CORE
    coap_free_type(COAP_RESOURCE,s);
#else
